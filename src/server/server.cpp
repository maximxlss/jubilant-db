...
Server::Server(std::filesystem::path base_dir, std::size_t worker_count)
  ...
  manifest_record_ = manifest_store_.Load().value_or(meta::ManifestStore::NewDefault("server-bootstrap"));
  ...
  btree_.emplace(storage::btree::BTree::Config{.pager = &pager_.value(),
