...
- [x] (2025-12-17 23:34Z) Completed integration coverage that exercises the shipped `jubildb_server`
      binary over TCP (strings, ints, bytes) across restarts to finish the end-to-end network path.
...
The networked transaction preview now runs through the released `jubildb_server` binary with remote
clients driving strings, ints, and bytes, and it survives a full stop/start cycle while preserving
data. Coverage spans the Python and C++ clients plus raw socket framing, and restart validation
keeps us honest about persistence. Remaining gaps are post-v0.0.2 polish items such as richer
protocol features and operational automation.
