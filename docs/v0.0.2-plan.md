...
- [ ] Define and document the minimal transaction wire format and compatibility constraints.
- [ ] Prototype Python client against a loopback stub server to validate framing and payloads.
- [ ] Implement C++ server network adapter that decodes requests and feeds `TransactionReceiver`.
...
- None yet; populate as implementation uncovers edge cases or protocol adjustments.
...
1. Document the wire contract in `docs/` (e.g., `docs/txn-wire-v0.0.2.md`), including request/response JSON schemas, framing rules (uint32 length prefix, network byte order), and examples for `set/get/del`. **Blocks:** none; **Unblocks:** steps 2, 3, 6.
