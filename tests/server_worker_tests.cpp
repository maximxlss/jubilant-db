...
TEST(WorkerTest, DoesNotCommitStagedWritesWhenAborted) {
  TransactionReceiver receiver{};
  LockManager lock_manager{};
  const auto dir = std::filesystem::temp_directory_path() / "jubilant-worker-abort";
  std::filesystem::remove_all(dir);
  Pager pager = Pager::Open(dir / "data.pages", jubilant::storage::kDefaultPageSize);
  ValueLog vlog(dir / "vlog");
  jubilant::storage::btree::BTree btree(jubilant::storage::btree::BTree::Config{
  ...
  Worker worker{
      "worker-abort", receiver, lock_manager, btree, btree_mutex, [&](TransactionResult result) {
        std::lock_guard guard(results_mutex);
        results.push_back(std::move(result));
        results_cv.notify_all();
      }};
  ...
  Record record{};
  record.value = static_cast<std::int64_t>(7);

  Operation set_op{.type = OperationType::kSet, .key = "alpha", .value = record};
  Operation assert_op{.type = OperationType::kAssertNotExists, .key = "alpha"};
  TransactionRequest request = BuildTransactionRequest(2, {set_op, assert_op});
  ...
  ASSERT_FALSE(results.empty());
  const auto& result = results.front();
  EXPECT_EQ(result.state, TransactionState::kAborted);
  ASSERT_EQ(result.operations.size(), 1U);
  EXPECT_EQ(result.operations.front().type, OperationType::kSet);
  EXPECT_FALSE(btree.Find("alpha").has_value());
}
